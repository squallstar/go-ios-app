...
#define BOTTOMBAR_COLLAPSEDELTA 73.0f
...
-(IBAction)tapRefresh:(id)sender {
    ...
    JSNotifier *notify = [[JSNotifier alloc]initWithTitle:@"Caricamento links.."];
    notify.accessoryView = activityIndicator;
    if (!sender) [notify show];
    
    [[API sharedAPI] getLinksByName:nil andCategoryId:0 success:^(NSArray *links) {
...
-(IBAction)tapSearch:(id)sender {
    SearchController *search = [[SearchController alloc] initWithNibName:@"SearchController" bundle:nil];
...
-(IBAction)tapLogout:(id)sender {
    ...
    [(GoAppDelegate*)[UIApplication sharedApplication].delegate showLogin];
}
